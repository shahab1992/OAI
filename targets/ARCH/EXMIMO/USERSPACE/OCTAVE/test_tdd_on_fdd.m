...
s = s*2;
s(1:length/2,1) = 1+1j; %put first chain on RX in first half
s(length/2+1:length,2) = 1+1j; %put second chain on RX in second half
...
figure(2)
plot(20*log10(abs(fft(s(1:length/2,1)))))
title('RX chain 0')

figure(3)
plot(20*log10(abs(fft(s(length/2+1:length,2)))))
title('RX chain 1')

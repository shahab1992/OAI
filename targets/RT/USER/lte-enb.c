...
void rx_rf_new(PHY_VARS_eNB *eNB,int *frame,int *subframe) {
  ...
  //LOG_I(PHY,"****** DEbug timestamp ****  %"PRId64"    \n\n\n",proc->timestamp_rx);
  ...
  proc->timestamp_rx = ts-eNB->ts_offset;
  //LOG_I(PHY,"****** DEbug timestamp after usrp ****  %"PRId64"    \n\n\n",proc->timestamp_rx);
  ...
  if (proc->first_rx == 1) {
    eNB->ts_offset = proc->timestamp_rx; // LOG_I(PHY,"****** DEbug timestamp first SF ****  %"PRId64"    \n\n\n",proc->timestamp_rx);
    proc->timestamp_rx=0;
  }
  else {
...
void tx_rf_new(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc_rxtx) {
  ...
  if ((SF_type == SF_DL) ||
    ...
    for (i=0; i<fp->nb_antennas_tx; i++)
      txp[i] = (void*)&eNB->common_vars.txdata[0][i][(((proc_rxtx->subframe_tx)%10)*siglen) - sf_extension];

    VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME( VCD_SIGNAL_DUMPER_FUNCTIONS_TRX_WRITE, 1 );
    VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME( VCD_SIGNAL_DUMPER_VARIABLES_TRX_WRITE_FLAGS,flags);
    txs = eNB->rfdevice.trx_write_func(&eNB->rfdevice,
				       proc_rxtx->timestamp_rx+proc_rxtx->ts_offset+(4*fp->samples_per_tti)-openair0_cfg[0].tx_sample_advance-sf_extension,
				       txp,
				       siglen+sf_extension,
				       fp->nb_antennas_tx,
				       flags);
    clock_gettime( CLOCK_MONOTONIC, &end_rf);
    end_rf_ts = proc_rxtx->timestamp_rx+proc_rxtx->ts_offset+(4*fp->samples_per_tti)-openair0_cfg[0].tx_sample_advance;
    if (recv_if_count != 0 ) {
      recv_if_count = recv_if_count-1;
      LOG_D(HW,"[From Timestamp %d to Timestamp %d] RTT_RF: %"PRId64"; RTT_RF\n", start_rf_prev_ts, end_rf_ts, clock_difftime_ns(start_rf_prev, end_rf));
      LOG_D(HW,"[From Timestamp %d to Timestamp %d] RTT_RF: %"PRId64"; RTT_RF\n",start_rf_prev2_ts, end_rf_ts, clock_difftime_ns(start_rf_prev2, end_rf));

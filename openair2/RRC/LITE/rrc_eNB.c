...
char buft[80];
strftime(buft,80,"%Y-%m-%d %H:%M:%S",localtime(&curTime.tv_sec));
...
#if defined(Rel10) || defined(Rel14)
sprintf(measReportLog,"At time %s:%3d  RSRP of Source %4ld and RSRQ of Source %4ld\n",buft,milli, measResults2->measResultPCell.rsrpResult,
		  measResults2->measResultPCell.rsrqResult);
#else
sprintf(measReportLog,"At time %s:%3d  RSRP of Source %4ld RSRQ of Source %4ld\n", buft,milli,measResults2->measResultServCell.rsrpResult,
		measResults2->measResultServCell.rsrqResult);
#endif

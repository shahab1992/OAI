...
int rrc_eNB_process_PAGING_IND(MessageDef *msg_p, const char *msg_name, instance_t instance)
{
  ...
  lte_frame_type_t frame_type = mac_xface->frame_parms->frame_type;
  ...
  for (uint16_t tai_size = 0; tai_size < S1AP_PAGING_IND(msg_p).tai_size; tai_size++) {
      ...
      if (enb_properties->properties[instance]->mcc == S1AP_PAGING_IND(msg_p).plmn_identity[tai_size].mcc
          && enb_properties->properties[instance]->mnc == S1AP_PAGING_IND(msg_p).plmn_identity[tai_size].mnc
          && enb_properties->properties[instance]->tac == S1AP_PAGING_IND(msg_p).tac[tai_size]) {
          for (uint8_t CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
              /* get nB from configuration */
              /* get default DRX cycle from configuration */
              Tc = (uint8_t)enb_properties->properties[instance]->pcch_defaultPagingCycle[CC_id];
              ...
              pcch_nB = (uint32_t)enb_properties->properties[instance]->pcch_nB[CC_id];
              switch (pcch_nB) {

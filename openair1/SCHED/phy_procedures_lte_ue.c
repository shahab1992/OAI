  ...
  for (i=0; i<dci_cnt; i++) {
	...
	(dci_alloc_rx[i].format != format0)) {
	  ...
	  (generate_ue_dlsch_params_from_dci(frame_rx,
          ...
          {
            ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->g_pucch += ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->harq_processes[ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->current_harq_pid]->delta_PUCCH;
          }
	...
	dump_dci(&ue->frame_parms, &dci_alloc_rx[i]);
	LOG_D(PHY,"[UE %d] *********** dlsch->active in subframe %d=> %d\n",ue->Mod_id,subframe_rx,ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->active);
#endif
...
int phy_procedures_UE_RX(PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,uint8_t eNB_id,uint8_t abstraction_flag,runmode_t mode,
  ...
  if (ue->dlsch[proc->subframe_rx&0x1][eNB_id][0])
    ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->active = 0;
  if (ue->dlsch[proc->subframe_rx&0x1][eNB_id][1])
    ue->dlsch[proc->subframe_rx&0x1][eNB_id][1]->active = 0;
  ...
  if (ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->active == 1) {
    VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_PDSCH_PROC, VCD_FUNCTION_IN);
    ue_pdsch_procedures(ue,
			proc,
			eNB_id,
			PDSCH,
			ue->dlsch[proc->subframe_rx&0x1][eNB_id][0],
			NULL,
			ue->pdcch_vars[subframe_rx & 0x1][eNB_id]->num_pdcch_symbols,
  ...
  if (ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->active == 1) {
    ...
    ue_dlsch_procedures(ue,
			...
			PDSCH,
			ue->dlsch[proc->subframe_rx&0x1][eNB_id][0],
			ue->dlsch[proc->subframe_rx&0x1][eNB_id][1],
			&ue->dlsch_errors[eNB_id],
			mode,
			abstraction_flag);
...
#if 0
  if(subframe_rx==5 &&  ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->harq_processes[ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->current_harq_pid]->nb_rb > 20){
  ...
  uint8_t          current_harq_pid        = ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->current_harq_pid;
  LTE_DL_UE_HARQ_t *current_harq_processes = ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->harq_processes[current_harq_pid];
  LTE_DL_UE_HARQ_t *harq_processes_dest    = ue->dlsch[(subframe_rx+1)&0x1][eNB_id][0]->harq_processes[current_harq_pid];

  harq_status_t *current_harq_ack = &ue->dlsch[proc->subframe_rx&0x1][eNB_id][0]->harq_ack[subframe_rx];

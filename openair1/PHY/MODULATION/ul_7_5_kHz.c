...
void remove_7_5_kHz_ul(PHY_VARS_eNB *phy_vars_eNB,uint8_t slot,uint16_t delay)
{ /*
  Same function as remove_7_5_kHz but taking into account the estimated delay - only for UFMC
  */
  ...
  slot_offset = (uint32_t)slot * phy_vars_eNB->lte_frame_parms.samples_per_tti/2-phy_vars_eNB->N_TA_offset+delay;

                  ...
                  uint8_t subframe)
{
  ...
  if (phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]!=NULL) {
    ...
    mcs = phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]->harq_processes[harq_pid]->mcs;

    // Button 0
    if(!phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]->harq_processes[harq_pid]->dl_power_off) {
      // we are in TM5
  ...
  pdsch_mag = (int16_t*) phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->dl_ch_mag0[0];

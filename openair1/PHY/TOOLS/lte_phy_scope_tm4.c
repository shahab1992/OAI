...
void phy_scope_UE(FD_lte_phy_scope_ue *form,
    ...
    if (phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]!=NULL) {
        harq_pid = phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]->current_harq_pid;
  if (harq_pid>=8)
    return;
    mcs0 = phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]->harq_processes[harq_pid]->mcs;
        // Button 0
  /*
  ...
  */
    ...
    fl_show_object(form->button_0);
      if (phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][1]!=NULL) {
        harq_pid = phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][1]->current_harq_pid;
  if (harq_pid>=8)
    return;
    mcs1 = phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][1]->harq_processes[harq_pid]->mcs;
    }
    if (phy_vars_ue->pdcch_vars[phy_vars_ue->current_thread_id][eNB_id]!=NULL) {
        num_pdcch_symbols = phy_vars_ue->pdcch_vars[phy_vars_ue->current_thread_id][eNB_id]->num_pdcch_symbols;
    }
    ...
    if (phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]!=NULL) {
      ...
      coded_bits_per_codeword0 = get_G(frame_parms,
              phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]->harq_processes[harq_pid]->nb_rb,
              phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]->harq_processes[harq_pid]->rb_alloc_even,
              get_Qm(mcs0),
              phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][0]->harq_processes[harq_pid]->Nl,
              num_pdcch_symbols,
              frame,
              subframe,
              beamforming_mode);
    ...
    if (phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][1]!=NULL) {
      ...
      coded_bits_per_codeword1 = get_G(frame_parms,
               phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][1]->harq_processes[harq_pid]->nb_rb,
               phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][1]->harq_processes[harq_pid]->rb_alloc_even,
               get_Qm(mcs1),
               phy_vars_ue->dlsch[phy_vars_ue->current_thread_id][eNB_id][1]->harq_processes[harq_pid]->Nl,
               num_pdcch_symbols,
               frame,
               subframe,
               beamforming_mode);
    ...
    chest_t = (int16_t**) phy_vars_ue->common_vars.common_vars_rx_data_per_thread[phy_vars_ue->current_thread_id].dl_ch_estimates_time[eNB_id];
    chest_f = (int16_t**) phy_vars_ue->common_vars.common_vars_rx_data_per_thread[phy_vars_ue->current_thread_id].dl_ch_estimates[eNB_id];
    pbch_llr = (int8_t*) phy_vars_ue->pbch_vars[eNB_id]->llr;
    pbch_comp = (int16_t*) phy_vars_ue->pbch_vars[eNB_id]->rxdataF_comp[0];
    pdcch_llr = (int8_t*) phy_vars_ue->pdcch_vars[phy_vars_ue->current_thread_id][eNB_id]->llr;
    pdcch_comp = (int16_t*) phy_vars_ue->pdcch_vars[phy_vars_ue->current_thread_id][eNB_id]->rxdataF_comp[0];
    pdsch_llr = (int16_t*) phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->llr[0]; // stream 0
    pdsch_llr1 = (int16_t*) phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->llr[1]; // stream 1
    pdsch_comp = (int16_t*) phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->rxdataF_comp0[0];
    //pdsch_comp = (int16_t*) phy_vars_ue->lte_ue_pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->rxdataF_ext[0];
    //pdsch_comp1 = (int16_t*) phy_vars_ue->lte_ue_pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->rxdataF_ext[1];
    pdsch_comp1 = (int16_t*) (phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->rxdataF_comp1[0][0])[0];
    //pdsch_comp1 = (int16_t*) (phy_vars_ue->lte_ue_pdsch_vars[eNB_id]->dl_ch_rho_ext[0][0])[0];
    pdsch_mag0 = (int16_t*) phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->dl_ch_mag0[0];
    pdsch_mag1 = (int16_t*) (phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->dl_ch_mag1[0][0])[0];
    pdsch_magb0 = (int16_t*) phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->dl_ch_magb0[0];
    pdsch_magb1 = (int16_t*) (phy_vars_ue->pdsch_vars[phy_vars_ue->current_thread_id][eNB_id]->dl_ch_magb1[0][0])[0];

                      ...
                      short coef)
{
  ...
  for (i = 0; i < frame_parms->nb_prefix_samples; i++) {
    ...
    for (aa=0; aa<frame_parms->nb_antennas_rx; aa++) {
      Re = ((int16_t*)ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id].dl_ch_estimates_time[eNB_id][aa])[(i<<2)];
      Im = ((int16_t*)ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id].dl_ch_estimates_time[eNB_id][aa])[1+(i<<2)];

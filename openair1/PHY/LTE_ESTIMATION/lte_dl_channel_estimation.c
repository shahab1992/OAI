                              ...
                              unsigned char symbol)
{
  ...
  int **dl_ch_estimates         =ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id].dl_ch_estimates[eNB_offset];
  ...
  for (aarx=0; aarx<ue->frame_parms.nb_antennas_rx; aarx++)
    for (p=0; p<ue->frame_parms.nb_antenna_ports_eNB; p++) {
      if (ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id].dl_ch_estimates[eNB_offset][(p<<1)+aarx])
        idft((int16_t*) &ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id].dl_ch_estimates[eNB_offset][(p<<1)+aarx][8],
             (int16_t*) ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id].dl_ch_estimates_time[eNB_offset][(p<<1)+aarx],1);
    }
        ...
        T(T_UE_PHY_DL_CHANNEL_ESTIMATE, T_INT(eNB_id), T_INT(ue->Mod_id),
          ...
          T_INT(0), T_BUFFER(&ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id].dl_ch_estimates_time[eNB_offset][0][0], 512  * 4));
#endif
